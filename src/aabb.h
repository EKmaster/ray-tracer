...
- Stores one interval per axis.
- Provides fast slab-based ray/box intersection.
- Forms the core primitive for BVH node bounds.
*/
...
class aabb {
  public:
    ...
    bool hit(const ray& r, interval ray_t) const {
        ...
        for (int axis = 0; axis < 3; axis++) {
            ...
            // Inverse direction avoids per-axis division in both t endpoints.
  ...
  private:
    void pad_to_minimums() {
        // Avoid zero-thickness boxes that can destabilize slab tests.
        double delta = 0.0001;
